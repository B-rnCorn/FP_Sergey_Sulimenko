function valid() {
  ...
  if (noValid.length !== 0) {
    ...
    if (resCookie && nameCookie && surnameCokie) {
      alert(
        `${review.name + " " + review.surname} , ваше сообщение обрабатывается!`
      );
    } else {
      alert(
        `${review.name + " " + review.surname} , спасибо за ваше сообщение!`
      );
      document.cookie = `alreadySend=true; path=/home/sergey/Projects/FP_Sergey_Sulimenko/FP_Sergey_Sulimenko/Task6/;`;
